...
public class KdTree<T extends KdTree.XYZPoint> {
	...
	public Collection<T> nearestNeighbourSearch(/*int K,*/ T value, double kDist) {
		if (value == null)
			return null;
		...
		if (leaf != null) {
			...
			while (node != null) {
				// Search node
				searchNode(value, node, /*K,*/ kDist, results, examined);
				node = node.parent;
			}
		}
	...
	private static final <T extends XYZPoint> void searchNode(T value, KdNode node, /*int K,*/ double kDist,
		...
		if (nodeDistance.compareTo(lastDistance) < 0) {
			if (nodeDistance>kDist && lastNode != null)
			//if (results.size() == K && lastNode != null)
				results.remove(lastNode);
			results.add(node);
		} else if (nodeDistance.equals(lastDistance)) {
			results.add(node);
		}else if (nodeDistance<kDist) { 
		...
		if (lesser != null && !examined.contains(lesser)) {
			...
			if (lineIntersectsCube)
				searchNode(value, lesser, /*K,*/ kDist, results, examined);
		}
		if (greater != null && !examined.contains(greater)) {
			...
			if (lineIntersectsCube)
				searchNode(value, greater, /*K,*/ kDist, results, examined);
		}
	}
	...
	public static class XYZPoint implements Comparable<XYZPoint> {
		...
		public double start_lat = Double.NEGATIVE_INFINITY;
		public double start_long = Double.NEGATIVE_INFINITY;
		public double end_lat = Double.NEGATIVE_INFINITY;
		public double end_long = Double.NEGATIVE_INFINITY;
		public int availability =  0;

		public XYZPoint(String linearID, String address ,
				double mid_x, double mid_y,
				double start_lat,double start_long,
				double end_lat,double end_long, int availability) {
			this.x = mid_x;
			this.y = mid_y;
			this.z = 0;
...
/*		private static final double euclideanDistance(XYZPoint o1, XYZPoint o2) {
		...
		public static final double euclideanDistance(XYZPoint o1, XYZPoint o2) {
		    ...
		    double dLat = Math.toRadians(o2.x-o1.x);
		    double dLng = Math.toRadians(o2.y-o1.y);
		    double sindLat = Math.sin(dLat / 2);
		    double sindLng = Math.sin(dLng / 2);
		    double a = Math.pow(sindLat, 2) + Math.pow(sindLng, 2)
		            * Math.cos(Math.toRadians(o1.x)) * Math.cos(Math.toRadians(o2.x));
